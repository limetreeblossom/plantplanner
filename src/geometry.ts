...
export function calcPolygonArea(points: Array<{ x: number; y: number }>, scale = SCALE): number {
...
export function polygonSelfIntersects(points: Array<{ x: number; y: number }>): boolean {
  const n = points.length;
  if (n < 4) return false;
  for (let i = 0; i < n; i++) {
    const i2 = (i + 1) % n;
